...
export default function Book({book}: {book: DbBook}) {
 ...
 ellipsizeMode="tail" className='text-white text-lg font-semibold'>{book.title}</Text>
            <Text numberOfLines={2} ellipsizeMode="tail" className='text-white/40 text-xs font-semibold' >{book.authors}</Text>
        </View>
        </View>
    </Link>
  )
