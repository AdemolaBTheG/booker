    ...
    <View className='flex-1  items-center px-4'>
        ...
        <View className="flex items-center justify-center w-full gap-4 mt-7">
            {readingTypes.map((type, index) => (
                <Pressable key={index} onPress={() => setIsFocused(index)}   className={`flex-row gap-2 items-center justify-center p-3 w-full border  rounded-2xl ${isFocused === index ? 'bg-cta border-white' : ' border-white/20'}`}>
                    <Text className='text-2xl text-white'>{type.emoji}</Text>
